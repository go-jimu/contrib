...
**Tech Stack:** Go 1.25 toolchain, `github.com/go-jimu/components v0.8.0`, `github.com/twmb/franz-go v1.21.1`, protobuf via `google.golang.org/protobuf/proto`, tests with Go `testing`, `github.com/stretchr/testify/require`, and build-tagged Kafka integration tests using `github.com/testcontainers/testcontainers-go/modules/kafka v0.42.0`.
...
- Build-tagged Kafka integration tests with testcontainers.
- README examples.
...
- Create `message/kafka/kafka_integration_test.go`: build-tagged testcontainers Kafka tests.
...
- [ ] integration real: publisher sends to a testcontainers Kafka broker and consumer receives through `Consumer.Run` -> handler observes original message fields.
- [ ] integration real: handler error against testcontainers Kafka broker -> retry topic receives failure record.
...
## Task 7: Kafka Integration Tests With Testcontainers

**Files:**
- Create: `message/kafka/kafka_integration_test.go`
- Test: `message/kafka/kafka_integration_test.go`

- [ ] **Step 1: Write build-tagged Kafka integration tests**
...
func (h integrationHandler) Listening() []message.Kind { return h.kinds }
...
func startKafka(t *testing.T) []string {
	...
	container, err := tckafka.Run(
		...
		tckafka.WithClusterID(fmt.Sprintf("jimu-%d", time.Now().UnixNano())),
	)
	require.NoError(t, err)
	t.Cleanup(func() {
...
// Intent: the exported publisher and consumer should interoperate through a real Kafka broker, not only through fakes.
func TestIntegrationPublishAndConsumeMessage(t *testing.T) {
	brokers := startKafka(t)
	topic := fmt.Sprintf("orders-success-%d", time.Now().UnixNano())
	client := newIntegrationClient(t, brokers, fmt.Sprintf("group-%d", time.Now().UnixNano()), topic)
	...
	defer cancelRun()
	require.NoError(t, consumer.Subscribe(integrationHandler{
	...
	select {
	case got := <-received:
		require.Equal(t, message.Kind(topic), got.Kind())
		require.Equal(t, "order-7", got.Key())
		require.Equal(t, map[string]string{"trace_id": "trace-1"}, got.Headers())
		require.Equal(t, &testdata.TestModel{Id: 7, Name: "paid"}, got.Payload())
...
// Intent: a handler error should be handed off to a real Kafka retry topic so source offsets can move after durable retry publication.
func TestIntegrationHandlerErrorPublishesRetryRecord(t *testing.T) {
	brokers := startKafka(t)
	sourceTopic := fmt.Sprintf("orders-retry-%d", time.Now().UnixNano())
	retryTopic := sourceTopic + ".retry"
	sourceClient := newIntegrationClient(t, brokers, fmt.Sprintf("source-group-%d", time.Now().UnixNano()), sourceTopic)
	retryClient := newIntegrationClient(t, brokers, fmt.Sprintf("retry-group-%d", time.Now().UnixNano()), retryTopic)
	...
	require.NoError(t, consumer.Subscribe(integrationHandler{
		...
		handle: func(context.Context, message.Message) error {
			return errors.New("temporary handler communication failure")
		},
	}))
	...
	msg, err := message.New(message.Kind(sourceTopic), &testdata.TestModel{Id: 8, Name: "retry"}, message.WithKey("order-8"))
	require.NoError(t, err)
	require.NoError(t, publisher.Publish(context.Background(), msg))

	retryRecord := pollOneRecord(t, retryClient, 30*time.Second)
	cancelRun()

	require.Equal(t, retryTopic, retryRecord.Topic)
	require.Equal(t, []byte("order-8"), retryRecord.Key)
	require.Equal(t, "1", headerValue(retryRecord.Headers, "jimu-retry-attempt"))
	require.Equal(t, sourceTopic, headerValue(retryRecord.Headers, "jimu-original-topic"))
	require.Equal(t, "handle", headerValue(retryRecord.Headers, "jimu-failed-stage"))
	require.ErrorIs(t, <-runErr, context.Canceled)
}
...
```bash
cd message/kafka && go test -tags=integration ./... -run 'TestIntegration' -count=1
```

Expected: PASS when Docker is available. If Docker is unavailable, report the environment failure and do not weaken the tests.

- [ ] **Step 3: Commit integration tests**

```bash
git add message/kafka/kafka_integration_test.go message/kafka/go.mod message/kafka/go.sum
git commit -m "test: add kafka message integration tests"
```

## Task 8: README, Release Matrix, And Verification
...
## Task 9: Project Knowledge Update
...
Expected: recent commits include scaffold, record mapping, publisher, retry/DLQ, consumer, integration tests, docs, and project knowledge commits.
...
- Release matrix and workspace: Tasks 1 and 8.
- Project knowledge update: Task 9.

Placeholder scan:

- No incomplete-marker text remains.
