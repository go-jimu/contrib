...
| --- | --- | --- | --- |
| Go | Root `go.mod` uses 1.21; `go.work` and CI use 1.25.x; provider modules use 1.24.0 or 1.25.0 | Provider implementation language | Matches the `go-jimu/components` ecosystem and current CI matrix. |
...
| GitHub Actions | `.github/workflows/ci.yml` with current checkout/setup-go/artifact/codecov actions | CI, coverage upload, package tag release | Runs the Go 1.25.x matrix and package-scoped tags without deprecated artifact actions. |
...
| --- | --- | --- | --- |
| `github.com/go-jimu/components` | v0.8.0 in all provider modules | Shared component interfaces | Defines the contracts this repo implements. |
